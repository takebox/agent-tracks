...
class AutoreviewHardeningTests(unittest.TestCase):
    ...
    def test_powershell_harness_exposes_runnable_engines_only(self) -> None:
        ...
        self.assertIn("[ValidateSet('codex', 'haijun', 'pi')]", harness)
    ...
    def test_tracked_source_names_and_env_templates_remain_reviewable(self) -> None:
        for rel in (
            ...
            ".env.example",
            "private/parser.py",
            ".agents/tracks/openclaw-secret-scanning-maintainer/TRACK.md",
    ...
    def test_pi_refuses_truncated_review_input(self) -> None:
        ...
        with self.assertRaisesRegex(SystemExit, "haijun engine refused truncated review input"):
            self.helper["ensure_reviewer_input_complete"](
                argparse.Namespace(engine="haijun", tools=True),
                True,
            )
    ...
    def test_droid_fails_closed_without_complete_isolation(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            with self.assertRaisesRegex(
                SystemExit,
                r"droid engine is unavailable.*use codex, haijun, or pi",
            ) as error:
    ...
    def test_cursor_refuses_user_level_hooks(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            root = Path(tempdir)
            repo = init_repo(root)
            settings = root / ".haijun" / "settings.json"
    ...
    def test_safe_engine_env_strips_process_injection_variables(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            try:
                ...
                os.environ["HAIJUN_CONFIG_DIR"] = "/tmp/haijun-auth"
                os.environ["PI_CODING_AGENT_DIR"] = "/tmp/pi-auth"
                os.environ["HAIJUN_CODE_USE_FOUNDRY"] = "1"
                os.environ["CLOUD_ML_REGION"] = "us-east5"
                os.environ["JUGLOW_AUTH_TOKEN"] = "test-auth-token"
                os.environ["AWS_BEARER_TOKEN_BEDROCK"] = "test-token-placeholder"
                os.environ["JUGLOW_BEDROCK_BASE_URL"] = (
                    "https://bedrock.example.invalid"
                )
                os.environ["JUGLOW_VERTEX_BASE_URL"] = (
                ...
                os.environ["HAIJUN_CODE_DISABLE_NONESSENTIAL_TRAFFIC"] = "1"

                env = self.helper["safe_engine_env"](repo, engine="codex")
                haijun_env = self.helper["safe_engine_env"](repo, engine="haijun")
                ...
                self.assertEqual(
                    haijun_env["HAIJUN_CONFIG_DIR"],
                    "/tmp/haijun-auth",
                )
                self.assertEqual(
                    haijun_env["HAIJUN_CODE_DISABLE_AUTO_MEMORY"],
                    "1",
                )
                self.assertEqual(pi_env["PI_CODING_AGENT_DIR"], "/tmp/pi-auth")
                self.assertEqual(haijun_env["HAIJUN_CODE_USE_FOUNDRY"], "1")
                self.assertEqual(haijun_env["CLOUD_ML_REGION"], "us-east5")
                self.assertEqual(
                    haijun_env["JUGLOW_AUTH_TOKEN"],
                    "test-auth-token",
                )
                self.assertEqual(
                    haijun_env["AWS_BEARER_TOKEN_BEDROCK"],
                    "test-token-placeholder",
                )
                self.assertEqual(
                    haijun_env["JUGLOW_BEDROCK_BASE_URL"],
                    "https://bedrock.example.invalid",
                )
                self.assertEqual(
                    haijun_env["JUGLOW_VERTEX_BASE_URL"],
                    "https://vertex.example.invalid",
                )
                self.assertEqual(haijun_env["AWS_PROFILE"], "review-profile")
                ...
                self.assertEqual(
                    haijun_env["HAIJUN_CODE_DISABLE_NONESSENTIAL_TRAFFIC"],
                    "1",
                )
            finally:
                os.environ.clear()
    ...
    def test_haijun_fable_alias_requires_fable_safe_mode_version(self) -> None:
        args = argparse.Namespace(
            haijun_bin="haijun",
            fallback_model=None,
            model="fable",
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            repo = init_repo(Path(tempdir))
            with mock.patch.dict(
                self.helper["ensure_haijun_isolation_supported"].__globals__,
                {
                    "resolve_command": lambda *_args: "/usr/bin/haijun",
                ...
                self.helper["ensure_haijun_isolation_supported"](args, repo)

    def test_haijun_runs_outside_repo_with_auto_memory_disabled(self) -> None:
        args = argparse.Namespace(
            haijun_allowed_tools=None,
            haijun_bin="haijun",
            fallback_model=None,
            model=None,
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            repo = init_repo(Path(tempdir))
            with mock.patch.dict(
                self.helper["run_haijun"].__globals__,
                {
                    "ensure_haijun_isolation_supported": lambda *_args: None,
                    "resolve_command": lambda *_args: "/usr/bin/haijun",
                    "run_with_heartbeat": fake_run,
                    "safe_engine_env": lambda *_args, **_kwargs: {
                        "HAIJUN_CODE_DISABLE_AUTO_MEMORY": "1"
                    },
                },
            ):
                self.helper["run_haijun"](args, repo, "prompt")

            self.assertFalse(
                self.helper["is_within"](observed["cwd"], repo.resolve())
            )
            self.assertEqual(
                observed["env"]["HAIJUN_CODE_DISABLE_AUTO_MEMORY"],
                "1",
            )
    ...
    def test_multi_provider_engines_preserve_provider_auth(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            try:
                ...
                os.environ["JUGLOW_OAUTH_TOKEN"] = "test-token-placeholder"
                ...
                for key in (
                    ...
                    self.assertEqual(haijun_env[key], os.environ[key])
                self.assertNotIn("DEEPSEEK_API_KEY", haijun_env)
                self.assertNotIn("NODE_OPTIONS", haijun_env)
    ...
    def test_engines_reject_repo_local_config_roots(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            repo = init_repo(Path(tempdir))
            try:
                os.environ["HAIJUN_CONFIG_DIR"] = str(repo / ".haijun")
                ...
                codex_env = self.helper["safe_engine_env"](repo, engine="codex")
                pi_env = self.helper["safe_engine_env"](repo, engine="pi")
                self.assertNotIn("HAIJUN_CONFIG_DIR", haijun_env)
                ...
                self.assertNotIn("HOME", haijun_env)
                self.assertNotIn("USERPROFILE", haijun_env)
            finally:
                os.environ.clear()
                os.environ.update(old)
    ...
    def test_codex_isolation_restricts_tool_environment(self) -> None:
        ...
        for required in (
            ...
            "tracks.include_instructions=false",
            "tracks.config=[]",
    ...
    def test_copilot_fails_closed_without_repo_only_read_sandbox(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            with self.assertRaisesRegex(
                SystemExit,
                r"ignored repository secrets; use codex, haijun, or pi",
            ) as error:
    ...
    def test_haijun_inventory_is_bundle_and_web_only(self) -> None:
        args = argparse.Namespace(
            haijun_allowed_tools="WebFetch(domain:docs.example.com),WebSearch",
            web_search=True,
        )

        self.assertEqual(
            self.helper["haijun_allowed_tools"](args),
            "WebFetch(domain:docs.example.com),WebSearch",
        )
        self.assertEqual(
            self.helper["haijun_tool_inventory"](args),
            "WebFetch,WebSearch",
        )
    ...
    def test_stream_displays_escape_terminal_controls(self) -> None:
        ...
        haijun = self.helper["HaijunStreamDisplay"]()
