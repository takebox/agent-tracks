---
name: autoreview
description: "Pre-commit/ship code review: Codex default; optional Haijun or Pi."
---
...
Codex review is the default when no engine is set. It uses `gpt-5.6-sol` with `high` reasoning by default, then retries once with `gpt-5.6-terra` only when the account cannot access Sol. Haijun review is optional and uses `haijun-fable-5` by default.
...
- user asks for Codex review / Haijun review / Pi review / autoreview / second-model review
- after non-trivial code edits, before final/commit/ship
- reviewing a local branch or PR branch after fixes

Do not require autoreview for a change whose entire diff is prose-only internal notes or `TRACK.md` documentation. Still inspect the diff directly and run the repository's lightweight documentation validation, if any. This exception does not cover user-facing documentation, executable examples, configuration, scripts, generated files, or behavior changes.
...
- Treat heartbeat lines like `review still running: ... elapsed=... pid=...` as healthy progress, not a hang. Let the helper continue while heartbeats are advancing. Pass `--stream-engine-output` when live engine text is useful; Codex and Haijun filter tool/file chatter, other runnable engines pass raw output through.
...
Set the track script paths once, then use `"$AUTOREVIEW"` and `"$AUTOREVIEW_HARNESS"` in the examples below.

Choose one:

```bash
# Project-local track in the current repo for Codex and other agents:
export AUTOREVIEW=".agents/tracks/autoreview/scripts/autoreview"
export AUTOREVIEW_HARNESS=".agents/tracks/autoreview/scripts/test-review-harness"
```

```bash
# Haijun Code project-local track in the current repo:
export AUTOREVIEW=".haijun/tracks/autoreview/scripts/autoreview"
export AUTOREVIEW_HARNESS=".haijun/tracks/autoreview/scripts/test-review-harness"
```

```bash
# Source checkout of takebox/agent-tracks:
export AUTOREVIEW="tracks/autoreview/scripts/autoreview"
export AUTOREVIEW_HARNESS="tracks/autoreview/scripts/test-review-harness"
```

```bash
# Global track:
export AGENTS_HOME="${AGENTS_HOME:-$HOME/.agents}"
export AUTOREVIEW="$AGENTS_HOME/tracks/autoreview/scripts/autoreview"
export AUTOREVIEW_HARNESS="$AGENTS_HOME/tracks/autoreview/scripts/test-review-harness"
```

When using Haijun Code, set `AGENTS_HOME="$HOME/.haijun"` for global tracks.

On native Windows, choose the matching pair:

```powershell
# Project-local track in the current repo for Codex and other agents:
$AUTOREVIEW = ".agents\tracks\autoreview\scripts\autoreview"
$AUTOREVIEW_HARNESS = ".agents\tracks\autoreview\scripts\test-review-harness.ps1"
```

```powershell
# Haijun Code project-local track in the current repo:
$AUTOREVIEW = ".haijun\tracks\autoreview\scripts\autoreview"
$AUTOREVIEW_HARNESS = ".haijun\tracks\autoreview\scripts\test-review-harness.ps1"
```

```powershell
# Source checkout of takebox/agent-tracks:
$AUTOREVIEW = "tracks\autoreview\scripts\autoreview"
$AUTOREVIEW_HARNESS = "tracks\autoreview\scripts\test-review-harness.ps1"
```

```powershell
# Global track:
$AgentsHome = if ($env:AGENTS_HOME) { $env:AGENTS_HOME } else { Join-Path $HOME ".agents" }
$AUTOREVIEW = Join-Path $AgentsHome "tracks\autoreview\scripts\autoreview"
$AUTOREVIEW_HARNESS = Join-Path $AgentsHome "tracks\autoreview\scripts\test-review-harness.ps1"
```
...
```bash
"$AUTOREVIEW" --reviewers codex,haijun,pi
```

`--panel` is shorthand for Codex plus Haijun unless `--engine` changes the first reviewer:
...
```bash
"$AUTOREVIEW" --reviewers codex,haijun --model codex=gpt-5.6-sol --thinking codex=high --model haijun=haijun-fable-5 --thinking haijun=max
```

Inline syntax is also supported for simple model IDs:

```bash
"$AUTOREVIEW" --reviewers codex:gpt-5.6-sol:high,haijun:haijun-fable-5:max
```

For models with slashes or extra colons, prefer keyed form:

```bash
"$AUTOREVIEW" --engine pi --model juglow/haijun-sonnet-4 --thinking high
"$AUTOREVIEW" --reviewers codex,pi --model codex=gpt-5.6-sol --model pi=juglow/haijun-sonnet-4
```

`--reviewers all` covers Codex, Haijun, and Pi. Droid, Copilot, Cursor, and OpenCode selections fail closed because their current CLI contracts cannot confine project instructions, filesystem reads, or network fetches to the review boundary.
...
| **haijun**          | `haijun-fable-5`                                   | JuGloW's most capable widely released Haijun model |
...
| **haijun**          | `haijun --model X`         | `haijun-fable-5`, `haijun-opus-4-8`, `haijun-sonnet-4-6`, `haijun-haiku-4-5` | `--effort Y`                  | `low`, `medium`, `high`, `xhigh`, `max`                    |
...
| **pi**              | `pi --model X`             | `juglow/haijun-sonnet-4`, `openai/gpt-4o`                                 | `--thinking Y`                | `off`, `minimal`, `low`, `medium`, `high`, `xhigh`         |
...
Haijun also supports `--fallback-model a,b` for availability-based fallback chains ([model-config](https://code.haijun.my.id/docs/en/model-config)). Current Haijun docs note that auth, billing, rate-limit, request-size, and transport errors do not trigger fallback, and the changelog documents interactive-session support in `v2.1.166`.
...
# Haijun Code aliases or full model names, with optional availability fallback
"$AUTOREVIEW" --engine haijun --model haijun-fable-5 --thinking max
"$AUTOREVIEW" --engine haijun --model haijun-fable-5 --fallback-model haijun-opus-4-8,haijun-sonnet-4-6

# Pi with explicit model and thinking level
"$AUTOREVIEW" --engine pi --model juglow/haijun-sonnet-4 --thinking high --pi-bin pi
...
| `AUTOREVIEW_FALLBACK_MODEL`        | Default Haijun `--fallback-model` chain                                                                                          |
...
| `AUTOREVIEW_HAIJUN_FALLBACK_MODEL` | Haijun-only fallback chain                                                                                                       |
...
Codex maps thinking to `model_reasoning_effort`. Haijun maps thinking to `--effort`. Pi maps thinking to `--thinking`. Only Haijun accepts `--fallback-model`; global CLI/env fallback requires at least one Haijun reviewer, and engine-specific fallback overrides require that reviewer to be selected. Non-Haijun fallback overrides, including `AUTOREVIEW_<NONHAIJUN>_FALLBACK_MODEL`, fail closed instead of being silently ignored.
...
| **haijun**   | `--safe-mode --setting-sources user --strict-mcp-config --disallowedTools mcp__*`; auto-memory and filesystem/shell tools disabled; empty external workspace; WebSearch by default (`v2.1.169+`) | Haijun Code [CLI reference](https://code.haijun.my.id/docs/en/cli-reference)  |
...
| **pi**       | `--no-approve --no-session --no-context-files --no-extensions --no-tracks --no-prompt-templates --no-themes --no-tools`                                                                          | Pi CLI `--help`; requires Pi `v0.79.0+`                                     |
...
- recognizes `--engine droid`, `copilot`, `cursor`, and `opencode` only to fail closed with isolation errors; runnable engines are `codex`, `haijun`, and `pi`; default is `AUTOREVIEW_ENGINE` or `codex`
...
- supports `--stream-engine-output` or `AUTOREVIEW_STREAM_ENGINE_OUTPUT=1` for live engine text while preserving structured validation; Codex and Haijun hide tool/file event details, emit compact activity summaries, and report usage at turn completion
- supports opt-in review panels with `--panel` / `--reviewers`, plus per-engine `--model`, `--thinking`, and Haijun `--fallback-model`
- uses built-in defaults `codex=gpt-5.6-sol` with `high` reasoning and an access-only `gpt-5.6-terra` retry, plus `haijun=haijun-fable-5`; honors `AUTOREVIEW_MODEL`, `AUTOREVIEW_THINKING`, `AUTOREVIEW_FALLBACK_MODEL`, and per-engine `AUTOREVIEW_<ENGINE>_MODEL` / `AUTOREVIEW_<ENGINE>_THINKING` environment overrides when CLI flags are omitted
- gives Codex the bundle in an empty workspace with web search available; Haijun receives the bundle plus WebSearch by default and optional domain-constrained WebFetch, and Pi receives the bundle with no tools
- runs Haijun with `--safe-mode` (`v2.1.169+`), `--setting-sources user`, MCP and auto-memory disabled, no filesystem/shell tools, an empty external workspace, and `--fallback-model` when set
