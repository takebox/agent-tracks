...
test("parses Haijun Code tool use and result blocks", () => {
  const doc = parse(
    [
  ...
  assert.equal(doc.format, "haijun");
...
test("keeps Haijun thinking content blocks", () => {
  const doc = parse(
    [
      JSON.stringify({
        type: "assistant",
...
test("CLI writes a one-file HTML export", async () => {
  ...
  await execFileAsync(process.execPath, [
    "tracks/session-viewer/scripts/session-viewer.ts",
    input,
    "--out",
    output,
  ]);
