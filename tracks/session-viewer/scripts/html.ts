...
(() => {
  ...
  const detect = (records) => {
    ...
    if (records.some((r) => isObject(r.value) && ["summary", "user", "assistant"].includes(String(r.value.type)) || (isObject(r.value?.message) && ["user", "assistant"].includes(String(r.value.message.role))))) return "haijun";
    return "unknown";
  };
  ...
  const isContextPacket = (event) => {
    ...
    return /^# AGENTS\\.md instructions/u.test(text)
      ...
      || text.includes("<tracks_instructions>")
  ...
  const render = () => {
    ...
    $("subtitle").textContent = doc?.sourcePath ? shortenHomePaths(doc.sourcePath) : "Load a Codex, Haijun, or OpenClaw/Pi JSONL file.";
