...
export const haijunImporter: SessionImporter = {
  format: "haijun",
  detect(records) {
  ...
  parse(records, sourcePath) {
    ...
    for (const record of records) {
      ...
      const parsedEvents = parseContentBlocks(`haijun-${record.line}`, content, timestamp, role);
    ...
    if (events.length === 0) {
      warnings.push("no Haijun message events found");
    }
