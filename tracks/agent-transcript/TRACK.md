...
```bash
tracks/agent-transcript/scripts/agent-transcript --help
```

Find a likely local session:

```bash
tracks/agent-transcript/scripts/agent-transcript find \
  --query "$PR_TITLE $BRANCH_OR_PR_URL" \
  --cwd "$PWD" \
  --since-days 14
```

`find` scans the newest 400 matching local JSONL logs by default across Codex, Haijun, Pi, and OpenClaw agent sessions. Use `--max-files N` for a wider local search.

In a downstream repo that syncs shared tracks under `.agents/tracks`, replace
`tracks/agent-transcript` with `.agents/tracks/agent-transcript`.

Render a PR/issue body section:

```bash
tracks/agent-transcript/scripts/agent-transcript render \
...
```bash
tracks/agent-transcript/scripts/agent-transcript preview \
  --session "$SESSION_JSONL" \
...
```bash
tracks/agent-transcript/scripts/agent-transcript append-body \
  --body /tmp/pr-body.md \
  --session "$SESSION_JSONL" \
...
```bash
node --test tracks/agent-transcript/scripts/agent-transcript.test.mjs
```
...
```bash
tracks/agent-transcript/scripts/agent-transcript html \
